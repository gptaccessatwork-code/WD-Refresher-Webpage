...
6.  Verify green 'All Assigned' tick; if missing → save anyway, flag for review
...
# Badge images in the right-panel Item Structure after assignment:
#   Good: <image xlink:href="…/qual_checkmark_16.png">  (green tick – all assigned)
#   Bad:  <image xlink:href="…/qual_bluebadge_24.png">  (blue circle – qty remaining)
# The blue badge has no aria-label, so we distinguish purely by href filename.
_BLUE_BADGE_XPATH = "//*[name()='image'][contains(@*[local-name()='href'],'qual_bluebadge_24')]"
_GREEN_TICK_XPATH = "//*[name()='image'][contains(@*[local-name()='href'],'qual_checkmark_16')]"
...
def verify_assignment(driver, item_number: str) -> Optional[str]:
    """
    Check assignment state by badge image href in the right-panel.

    After assignment Oracle renders one badge per child item:
        Good: qual_checkmark_16.png  (green tick — fully assigned)
        Bad:  qual_bluebadge_24.png  (blue circle — quantity still unassigned)

    The blue badge has no aria-label so we distinguish by href filename only.

    Steps:
      1. Wait for at least one green tick — confirms panel has finished rendering.
      2. Check for any remaining blue badges.
         Found  → return reason string (written to wd_exceptions.xlsx).
         None   → return None (success).
    """
    log.info("Verifying assignment state via badge images…")

    # Wait for at least one green tick to confirm the panel has rendered
    try:
        _w(driver, MEDIUM_WAIT).until(
            EC.presence_of_element_located((By.XPATH, _GREEN_TICK_XPATH)))
    except TimeoutException:
        reason = "No green checkmark badges found after assignment – state unknown"
        log.warning("%s – %s", item_number, reason)
        return reason

    blue_badges = driver.find_elements(By.XPATH, _BLUE_BADGE_XPATH)
    if blue_badges:
        count  = len(blue_badges)
        reason = (f"{count} item(s) still show blue quantity badge "
                  f"(qual_bluebadge_24) – not fully assigned")
        log.warning("%s – %s", item_number, reason)
        return reason

    log.info("✓ All items show green checkmark for %s.", item_number)
    return None
